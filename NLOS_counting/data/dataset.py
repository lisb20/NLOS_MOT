...
class TrackingDataset(Dataset):
    def __init__(self,
        ...
        self.dirs = []
        for peo in [2,3,4,5]:
            self.dirs += [os.path.join(str(peo), d) for d in os.listdir(os.path.join(self.dataset_dir, str(peo)))]
    ...
    def __getitem__(self, idx):
        ...
        route = mat_file['route'][start_frame:start_frame + self.num_frames]  # (T, npeo, 2)    baseline->(T,10)
        ## t * n * 2 -> t * 2n
        npeo = route.shape[1]
        ## label为npeo 的onehot
        # label = np.zeros((4,))
        ...
        # route = np.concatenate((route, np.ones((route.shape[0], 10 - route.shape[1])) * 0.5), axis=1)
        # assert route.shape[1] == 10 and route.shape[0] == self.num_frames
        map_size = mat_file['map_size']  # (1, 2)
        ## mapsize ->[mapsize * 5]
        # map_size = np.tile(map_size, (1, 5))  # (1,4)

        return torch.from_numpy(video), torch.from_numpy(label).float(), torch.from_numpy(map_size).float()
