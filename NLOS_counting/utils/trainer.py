...
class Trainer_tracking(Trainer_Base):
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.schedule_cfgs['max_epoch']):
            ...
            if self.dist_cfgs['local_rank'] == 0:
                wandb.log({f"train_acc": train_metric,
                           f"train_loss": train_loss}, step=epoch + 1)
                wandb.log({f"val_acc": val_metric,
                           f"val_loss":val_loss}, step=epoch + 1)
    ...
    def train(self, epoch):
        ...
        for step in range(len_loader):
            ...
            acc = (idx == labels).sum() / batch_size

            acc_recorder.update(acc.item(), batch_size)
